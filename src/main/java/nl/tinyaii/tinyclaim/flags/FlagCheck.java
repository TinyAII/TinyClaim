...
public class FlagCheck {
    ...
    public FlagCheck(TinyClaimPlugin plugin) {
        this.plugin = plugin;
    ...
    private Boolean resolveEnv(Claim claim, String flagName) {
        ...
        // 从 config 读全局默认
        return plugin.getConfig().getBoolean("flags.default-environment." + flagName,
    ...
    private Boolean resolvePri(Claim claim, String flagName) {
        Boolean overridden = claim.getFlagOverride(flagName);
        if (overridden != null) return overridden;
        return plugin.getConfig().getBoolean("flags.default-privilege." + flagName,
