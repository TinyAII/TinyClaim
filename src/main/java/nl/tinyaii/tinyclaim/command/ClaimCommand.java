package nl.tinyaii.tinyclaim.command;
...
import nl.tinyaii.tinyclaim.data.Cuboid;
import nl.tinyaii.tinyclaim.flags.Flags;
...
public class ClaimCommand implements CommandExecutor {
    ...
    public ClaimCommand(TinyClaimPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    private boolean create(Player p, String name) {
        nl.tinyaii.tinyclaim.command.SelectionManager sel = plugin.getSelectionManager();
    ...
    private boolean doCreate(Player p, String name, Cuboid cub,
                             nl.tinyaii.tinyclaim.command.SelectionManager sel) {
        ...
        int maxXZ = plugin.getConfig().getInt("settings.max-size-xz", 256);
        int maxY = plugin.getConfig().getInt("settings.max-size-y", 256);
        int minSize = plugin.getConfig().getInt("settings.min-size", 1);
        ...
        // 重叠检测
        if (plugin.getConfig().getBoolean("settings.overlap-forbidden", true)) {
        ...
        // 经济扣费
        double cost = plugin.getConfig().getDouble("settings.economy.claim-cost", 0);
        if (cost > 0) {
            String err = plugin.getEcoBridge().tryWithdraw(p, cost);
        ...
        mgr.addClaim(claim);
        plugin.getStorage().saveClaim(claim);
    ...
    private int claimLimit(Player p) {
        ...
        int limit = plugin.getConfig().getInt("settings.max-claims-default", 3);
        for (String perm : new String[]{"svip", "vip", "default"}) {
            if (p.hasPermission("claim.limit." + perm)) {
                int v = plugin.getConfig().getInt("settings.max-claims-" + perm, limit);
                if (v > limit) limit = v;
            }
        }
        return limit;
    ...
    private boolean delete(Player p, boolean confirmed) {
        ...
        mgr.removeClaim(c);
        plugin.getStorage().deleteClaim(c.getId());
    ...
    private boolean resolve(Claim c, String name) {
        ...
        boolean def = Flags.ENV_FLAGS.containsKey(name)
                ? plugin.getConfig().getBoolean("flags.default-environment." + name, Flags.ENV_FLAGS.get(name))
                : plugin.getConfig().getBoolean("flags.default-privilege." + name, Flags.PRI_FLAGS.get(name));
        return def;
    }
    ...
    private boolean panel(Player p) {
        new nl.tinyaii.tinyclaim.gui.GlobalPanelGui(plugin, p.getUniqueId()).open(p);
        return true;
    }
    ...
    private boolean child(Player p, String[] args) {
        ...
        if (op.equals("创建") || op.equals("create")) {
            ...
            plugin.getStorage().saveClaim(childClaim);
    ...
    private boolean template(Player p, String[] args) {
        ...
        org.bukkit.configuration.ConfigurationSection tpl = plugin.getConfig().getConfigurationSection("templates." + args[2]);
        if (tpl == null) {
            p.sendMessage(Messages.color("&c找不到模板 &e" + args[2] + "&c。可用: " + String.join(", ", plugin.getConfig().getConfigurationSection("templates").getKeys(false))));
            return true;
        }
        int applied = 0;
    ...
    private boolean expand(Player p, String[] args) {
        ...
        } else if (plugin.getConfig().getBoolean("settings.overlap-forbidden", true)) {
