...
public class SelectionManager {
    ...
    public SelectionManager(TinyClaimPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    public String onRightClick(Player p, Location loc) {
        ...
        if (a == null) {
            ...
            sel2.remove(u);
            stopPreview(u);
            p.sendMessage(nl.tinyaii.tinyclaim.util.Messages.color(
    ...
    private void startPreview(Player p) {
        ...
        long durationMs = plugin.getConfig().getInt("settings.display-duration-seconds", 10) * 1000L;
    ...
    private void drawNearbyClaims(Player p) {
        ...
        int radius = plugin.getConfig().getInt("settings.selection-preview-radius", 64);
    ...
    private void drawBox(Player p, Cuboid cub) {
        if (cub == null) return;
        String style = plugin.getConfig().getString("settings.boundary-style", "faces");
